...
# 10 / 14 / 2019 - Being a Producer

Okay, so the thing is starting to feel daunting, but that's okay. We have the controller for the astronaut and the space ship.

This works fine for desktop PC, but will need some adjustments in VR. We will want to have the camera locked in place. So we will have two version, Play in VR, and just Play.

So yeah, there are some rough spots, but we can get through this. We have both controllers, so now we have to set up both of the worlds, one is the planet flying world, and the other is the surface world.

Let's get things set up to a point, where we just need art, and then craft fun game mechanics of trying to survive in out space. The art will be critical to the look and feel of the game, but we don't have that yet.

But what we can focus on is procedurally generating planets, and then using the spaceship to fly around and get readings from different planets as to what the planet is composed of.

How does Space Engine do things in VR? Don't they also stay locked in one position? Might need to buy the game to find out.

In any case don't panic.

Set up:

1. Procedural planet generation (use the prototyping asset)
2. Be able to fly around these planets
3. When clicking on some landing button, be transported to a platform, which represents that planet


In the end, the style will overcome everything, so just relax and work through this. We want a full game loop going.

Instead of trying to create an ultra realistic game with every feature, create a unique style, and a singular game only an indie producer can make.

Again, go for the style of Forager but in deep space...

# 10 / 14 / 2019 - So What's Next?

Seems like there is a lot to do. But how can we go about getting started and making the largest difference?

Well we can get quite lost in modeling, and also level design, so for now we should only work on stuff "we get to keep forever", so nothing that is going to require extensive rework.

And also work on stuff, which gets us close to the Itch.io release

Here is a nice third person controller: https://assetstore.unity.com/packages/templates/systems/3rd-person-controller-fly-mode-28647

The terrain mode can actually be pretty bar, with just building.

We might need to go into space, and establish the basic space actions in space.

# 10 / 14 / 2019

Checking out the success of Noita

https://store.steampowered.com/app/881100/Noita/

Only 1 month on Steam, and already 2K reviews. They might have carried the crowd with them from Itch.io

Which goes to show start on Itch.io, and build up a crowd, especially a mailing list / discord, so can announce game updates, and when the game is going to be released to Steam

Check out also the game "Moonlight": https://store.steampowered.com/app/606150/Moonlighter/

Try to achieve this sense of contrast and art, in the featured screenshots, since this is what sells the game

# 10 / 13 / 2019 - MVP Assesment
...
If we are working with code then work on the procedural planet system, if we are working with modeling then work on the astronaut
...
* So now we want to give the player something to do, and that will be mining, (foraging), classic gameplay element, which always works, people love hoarding stuff
...
So we already have a lot done. We have a much smoother transition between the astronaut and the ship. 
...
Also if players mine something rare (variable reward schedule) perhaps this could facilitate mineral trade between players.
